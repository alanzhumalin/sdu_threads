...
export default function FabricImageEditor({ src, fileName, onCancel, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    c.on("mouse:move", (opt: any) => {
      ...
      const currentMode = modeRef.current;
      const ratio = currentMode === "crop" ? cropRatioValue(cropPresetRef.current) : null;
  ...
  useEffect(() => {
    ...
    const container = containerRef.current;
    if (!container) return;
    const obs = new ResizeObserver(() => resizeToFit());
  ...
  const topHint = useMemo(() => {
    ...
    if (mode === "crop") return "Выделите область. Примените кадрирование.";
    ...
    <div className="flex flex-col w-full h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
        <div className="space-y-0.5">
          <p className="text-white font-semibold text-sm">Редактирование</p>
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button
            ...
            onClick={() => {
              const c = canvasRef.current;
              if (!c) return;
              const next = clamp(c.getZoom() * 0.9, 0.25, 6);
          ...
          <button
            ...
            onClick={() => {
              const c = canvasRef.current;
              if (!c) return;
              const next = clamp(c.getZoom() * 1.1, 0.25, 6);
              c.zoomToPoint(new fabric.Point(c.getWidth() / 2, c.getHeight() / 2), next);
              setZoom(Number(next.toFixed(2)));
              c.requestRenderAll();
              scheduleViewSnapshot();
